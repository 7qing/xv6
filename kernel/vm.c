...
int supermappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa,
                  int perm) {
  uint64 a, last;
  pte_t *pte;
  //printf("va: %p\n", (void *)va);
  ...
  for (;;) {
    ...
    *pte = PA2PTE(pa) | perm |PTE_SUPER| PTE_V;
    if (a == last)
      break;
    a += SUPERPGSIZE;
    pa += SUPERPGSIZE;
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for (a = oldsz; a < newsz; a += sz) {
    if ((a % SUPERPGSIZE) == 0 && a + SUPERPGSIZE < newsz) {
      sz = SUPERPGSIZE;
      mem = superKalloc();
      //printf("uvmalloc: va: %p allocating super page at %p\n", (void *)a, mem);
    }else{
      sz = PGSIZE;
      mem = kalloc();
      //printf("uvmalloc: va: %p allocating page at %p\n", (void *)a, mem);
    }
    if (mem == 0) {
